...
transnistria_all = bbox %>% 
            opq(timeout = 900) %>%
            add_osm_feature(
key = "natural",
value = available_tags("natural"))%>%
osmdata_sf()

ggplot()+
geom_sf(data = 
transnistria_all$osm_polygons, aes())+
  geom_sf(data = 
transnistria_all$osm_multipolygons, 
aes(fill=natural))+
  theme_bw()

map_polygons = st_read("map.osm", 
            layer = 'multipolygons', quiet = TRUE)
map_lines = st_read("map.osm", 
            layer = 'lines’,quiet = TRUE)
map_polygons = map_polygons %>% 
filter(!is.na(natural) | !is.na(building))
map_lines = map_lines %>% 
	      filter(highway == "footway")
map_polygons = st_intersection(map_polygons, park_sf)
map_lines = st_intersection(map_lines, park_sf)

map_polygons$geometry = map_polygons$geometry %>% 
s2::s2_rebuild() %>%sf::st_as_sfc()
map_lines$geometry = map_lines$geometry %>% 
s2::s2_rebuild() %>% sf::st_as_sfc()
map_polygons = map_polygons %>% 
...
wood_area = sum(st_area(map_polygons %>% 
filter(natural=="Древесные насаждения")))
water_area = st_area(map_polygons %>% 
filter(natural=="Водные объекты"))
building_area = sum(st_area(map_polygons %>% 
...
sum_tabl = summary_table %>% gt(rowname_col = "name") %>% 
tab_header(  
title = "Сводная таблица типов ландшафта парка «transnistria»",  
